...
class Tree234
{
    public Node root = new Node(); // Создание корневого узла
    ...
    private void inOrder(Node thisNode)
    {
        if(thisNode.isLeaf())
        {
            int num = thisNode.getNumItems();

            for(int i = 0; i < num; i++)
            {
                sortArr[nElems++] =  thisNode.getItem(i).dData;
            }
        ...
        for(int j = 0; j < numItems + 1; j++)
        {
            ...
            if(nextNode != null)
            {
                ...
                if(j < numItems)
                {
                    sortArr[nElems++] = (thisNode.getItem(j).dData);
                }
            }
            else
            {
