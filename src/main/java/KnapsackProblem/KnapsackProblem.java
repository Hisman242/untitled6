...
public class KnapsackProblem {
    ...
    private int[] arr;
    //При создании класса принимаем конструктор с весом предметов
...
//int targetWeight- это сколько ещё нужно добрать до нужного веса.
        ...
        if(targetWeight==0){
            ...
            return true;
        } if(indexArr == arr.length && targetWeight != 0){
        ...
        //Проверка на то, что текущий индекс не вышел за границы массива.(Излишняя проверка, можно удалить)
...
//                    попробовали взять текущий предмет,но путь с ним не дал решения,тогда попробуем его не брать.
                ...
                //Предмет слишком большой и не поместиился  визначальное условиие(Переходим к следующему)
                return knapsack(targetWeight, indexArr + 1) ;
            }
        }
        //Запасной возврат фолс, в случае если ниичего до этого не сработало
        return false;
    }
